...
| `app/(app)/requests/page.tsx` | All requests, grouped by requestor (most recently active first), each group collapsible, plus a Collapse all/Expand all toggle | `requireRole("owner","manager")` |
...
| `Modal.tsx` | Generic confirm/cancel modal — `fixed inset-0`, centered, used everywhere. Renders via `createPortal(..., document.body)`, so it's immune to being trapped by any ancestor's `backdrop-filter`/`transform`/`filter` (those properties make an element a containing block for `position: fixed` descendants — without the portal, nesting a Modal caller inside such an ancestor makes the modal render tiny/off-center relative to that ancestor instead of centered on the viewport, and its scrim stops blocking outside clicks). This bit twice before the portal fix: once with a scrapped grouped/collapsing Requests variant nested inside `.chit`, again when `.content-scrim` (see below) was added as a page-wide `backdrop-filter` wrapper. The portal makes the DOM position of the `Modal` caller irrelevant going forward — call it from anywhere. |
...
`.content-scrim` (wraps page content in `app/(app)/layout.tsx` and `app/login/page.tsx`) is the translucent layer between that photo and the actual UI. Its tint/blur/edge-fade live entirely on a `::before` pseudo-element (`position: absolute; inset: 0; z-index: -1`, masked with a `linear-gradient` on all 4 sides so it fades toward the photo instead of ending in a hard edge), while `.content-scrim` itself just sets `position: relative; isolation: isolate` — no background, no mask. Real content (chits, fields, buttons) is a normal-flow child of `.content-scrim`, so it paints in front of `::before` and is **never masked** — only the backdrop fades, never the UI on top of it. `isolation: isolate` scopes that negative-`z-index` trick to `.content-scrim` alone (so it doesn't compete with `.aurora`'s own `z-index: -10` elsewhere in the tree) without making `.content-scrim` a `position: fixed` containing block — unlike `backdrop-filter`, `isolation` doesn't trigger that, which is what keeps `Modal`'s portal fix (above) unnecessary-but-safe here rather than mandatory.
