...
- **A few real bugs caught along the way**: a critical Next.js CVE (patched via version bump), a hardcoded secret that almost got committed, a git repo that turned out to be rooted at the whole home directory instead of just this project, several paddings silently doing nothing because they used spacing classes (`p-5.5`, `pt-4.5`) that don't exist in Tailwind's default scale, and a grouped/collapsing card layout tried for the owner/manager Requests screen whose edit modal ended up trapped inside a `backdrop-filter` card (tiny, off-center, and not blocking background clicks — so more than one item could be "open" for edit at once). Rather than patch around it, the Requests screen was simplified to reuse the exact same flat `RequestList`/`RequestChit` components as the worker's My Requests page, so the two views can't drift apart again.
