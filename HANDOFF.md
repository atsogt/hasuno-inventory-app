...
- **A few real bugs caught along the way**: a critical Next.js CVE (patched via version bump), a hardcoded secret that almost got committed, a git repo that turned out to be rooted at the whole home directory instead of just this project, several paddings silently doing nothing because they used spacing classes (`p-5.5`, `pt-4.5`) that don't exist in Tailwind's default scale, and the owner/manager Requests screen's edit modal rendering trapped inside a `backdrop-filter` card (tiny, off-center, and not blocking background clicks — so more than one item could be "open" for edit at once) — fixed by lifting edit state up and rendering the modal as a sibling, matching how the worker's request list already did it.
